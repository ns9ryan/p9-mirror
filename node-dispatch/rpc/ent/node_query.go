...
type NodeQuery struct {
	...
	withOperatorNodes *OperatorNodeQuery
	withDispatchTasks *DispatchTaskQuery
...
// QueryOperatorNodes chains the current query on the "operator_nodes" edge.
func (_q *NodeQuery) QueryOperatorNodes() *OperatorNodeQuery {
	query := (&OperatorNodeClient{config: _q.config}).Query()
...
// QueryDispatchTasks chains the current query on the "dispatch_tasks" edge.
func (_q *NodeQuery) QueryDispatchTasks() *DispatchTaskQuery {
	query := (&DispatchTaskClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *NodeQuery) Clone() *NodeQuery {
	...
	return &NodeQuery{
		...
		withOperatorNodes: _q.withOperatorNodes.Clone(),
		withDispatchTasks: _q.withDispatchTasks.Clone(),
...
// WithOperatorNodes tells the query-builder to eager-load the nodes that are connected to
// the "operator_nodes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *NodeQuery) WithOperatorNodes(opts ...func(*OperatorNodeQuery)) *NodeQuery {
	query := (&OperatorNodeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOperatorNodes = query
	return _q
}

// WithDispatchTasks tells the query-builder to eager-load the nodes that are connected to
// the "dispatch_tasks" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *NodeQuery) WithDispatchTasks(opts ...func(*DispatchTaskQuery)) *NodeQuery {
	query := (&DispatchTaskClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDispatchTasks = query
	return _q
}
...
func (_q *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.withOperatorNodes != nil,
			_q.withDispatchTasks != nil,
		}
	)
	...
	if query := _q.withOperatorNodes; query != nil {
		if err := _q.loadOperatorNodes(ctx, query, nodes,
			func(n *Node) { n.Edges.OperatorNodes = []*OperatorNode{} },
			func(n *Node, e *OperatorNode) { n.Edges.OperatorNodes = append(n.Edges.OperatorNodes, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDispatchTasks; query != nil {
		if err := _q.loadDispatchTasks(ctx, query, nodes,
			func(n *Node) { n.Edges.DispatchTasks = []*DispatchTask{} },
			func(n *Node, e *DispatchTask) { n.Edges.DispatchTasks = append(n.Edges.DispatchTasks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NodeQuery) loadOperatorNodes(ctx context.Context, query *OperatorNodeQuery, nodes []*Node, init func(*Node), assign func(*Node, *OperatorNode)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(operatornode.FieldNodeID)
	}
	query.Where(predicate.OperatorNode(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(node.OperatorNodesColumn), fks...))
	}))
...
func (_q *NodeQuery) loadDispatchTasks(ctx context.Context, query *DispatchTaskQuery, nodes []*Node, init func(*Node), assign func(*Node, *DispatchTask)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Node)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(dispatchtask.FieldNodeID)
	}
	query.Where(predicate.DispatchTask(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(node.DispatchTasksColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
