...
func (l *GetCurrencyLogic) GetCurrency(req *types.GetCurrencyRequest) (resp *types.GetCurrencyResponse, err error) {
	...
	// 获取当前语言的货币名称
	name := i18n.TG(l.ctx, i18n.CodePlatform, "base", result.Currency.NameKey)
	...
	return &types.GetCurrencyResponse{
		CurrencyInfo: types.CurrencyInfo{
			...
			AmountFactor: result.Currency.AmountFactor, // 金额换算倍率, 如 USD=100, VND=1
			Status:       result.Currency.Status,       // 状态: 1启用, 2停用
			SortNo:       result.Currency.SortNo,       // 排序值, 数值越小越靠前
		},
	}, nil
}
